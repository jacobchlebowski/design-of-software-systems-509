...
export const handler = async (event, context, callback) => {
  ...
  let reviewPurchasesBuyer = (username) => {

    return new Promise((resolve, reject) => {
      pool.query("SELECT * FROM Items WHERE isFulfilled = ?", ["true"], (err, rows) => { //get items associated with seller
        if ((rows) && (rows.length > 0)) {
          let newRows = []
          for (let i = 0; i < rows.length; i++) {
            if (getHighestBidder(rows[i].bids) === username) {
              newRows.push(rows[i])
            }
          }
  ...
  //FORMAT DATES FOR CHECKING FOR INACTIVE ITEMS
  ...
  try {
    ...
    status = await reviewPurchasesBuyer(info.username);
    if (status != 400) {
      console.log(status)
