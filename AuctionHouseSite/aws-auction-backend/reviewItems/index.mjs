...
export const handler = async (event, context, callback) => {
  ...
  let reviewItems = (user) => {
    
    return new Promise((resolve, reject) => {
      pool.query("SELECT * FROM Items WHERE sellerUsername=?", [user], (err, rows) => { //get items associated with seller
  ...
  try {
    ...
    let status;
    status = await reviewItems(info.sellerUsername);
    if (status != 400) {
