...
export const handler = async (event, context, callback) => {
  ...
  let createSeller = (username,password) => {
    ...
    return new Promise((resolve, reject) => {
      pool.query( "INSERT INTO Users SET username=?, password=?, accountType=?, balance=? ,isClosed=? ,items=? ,bids=?", [username, password, "Seller", "0", "false",null,null], (err) => {
        if (err) {
          return resolve(401);
        }
        else {
  ...
  let openBuyer = (username,password) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      pool.query( "INSERT INTO Users SET username=?,password=?, accountType=?, balance=?,isClosed=?,items=?,bids=?", [username, password,"Buyer", "0", "false",null,null], (err) => {
        if (err) {
          return resolve(402);
        }
        else {
